...
2. Sample multiple frames to lock the watermark geometry.
3. Decode and process frames locally with reverse alpha blending.
4. Optionally apply a small Canvas residual cleanup pass for video-compression ringing.
5. Re-encode H.264 through native WebCodecs and copy the source audio packets when compatible.

Current scope is deliberately conservative: common 1080p and 720p diamond layouts are the main target, and low-confidence clips fail or skip uncertain frames instead of blindly modifying the wrong region.

**The video file is not uploaded.** The page does load the pinned Mediabunny browser module from jsDelivr; video processing itself stays on-device. Current Chrome/Edge are the primary browser target because Video Light needs H.264 WebCodecs support.

Video Light v1 intentionally does **not** bundle ffmpeg.wasm or a neural denoise model. A later optional enhanced mode can add WebGPU/WASM FDnCNN cleanup without changing the local-only architecture.
...
- **Small default surface** — the image tool and userscript stay compact; Video Light avoids ffmpeg.wasm and bundled ML in v1.
...
Video Light v1 targets supported visible **Gemini/Veo diamond** video watermarks. It is experimental and does not yet promise the same coverage or cleanup quality as dedicated native video tools on heavily compressed or unusual clips.
