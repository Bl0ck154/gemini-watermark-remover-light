...
function patchSelectConfig(source, indent = '  ', quote = '"') {
  const q = quote;
  const pattern = new RegExp(`${indent.replace(/ /g, ' ')}async function selectConfig\\(imageData\\) \\{[\\s\\S]*?\\n${indent}\\}\\n\\n${indent}async function scanBottomRightConfig`);
  ...
  return replaceOnce(source, pattern, replacement, 'selectConfig');
}

const upstreamMaps = await fetch(upstreamMapsUrl).then((response) => {
  assert(response.ok, `Failed to fetch upstream maps: ${response.status}`);
  return response.text();
});
const map36 = quantizeUpstream36Map(upstreamMaps);
...
userscript = replaceOnce(
  ...
  '// @description  Lightweight download-focused Gemini remover with standalone maps, current v2 profile support and confidence-gated matching',
  'userscript description'
);
if (!userscript.includes('"36-v2":')) {
  userscript = replaceOnce(
    userscript,
    '  const EMBEDDED_ALPHA_MAPS_U8 = Object.freeze({\n    "24-preview":',
    `  const EMBEDDED_ALPHA_MAPS_U8 = Object.freeze({\n    "36-v2": "${map36}",\n    "24-preview":`,
    '36-v2 userscript map insertion'
  );
}
if (!userscript.includes('MIN_MULTI_SCORE_GAP')) {
  userscript = replaceOnce(
    userscript,
    '  const MIN_MULTI_CANDIDATE_SCORE = 0.002;\n',
    '  const MIN_MULTI_CANDIDATE_SCORE = 0.002;\n  const MIN_MULTI_SCORE_GAP = 0.0005;\n',
    'userscript multi-candidate gap constant'
  );
}
userscript = replaceOnce(
  userscript,
  /  function getCandidateConfigs\(width, height\) \{[\s\S]*?\n  \}\n\n  function scoreConfig/,
  userscriptCandidateBlock(),
  'userscript candidate resolver'
);
userscript = patchSelectConfig(userscript, '  ', '"');
userscript = replaceOnce(
  userscript,
  '      { mapKey: "24-preview", size: 24, alphaGain: 0.55 },\n      { mapKey: "48", size: 48, alphaGain: 0.55 },',
  '      { mapKey: "24-preview", size: 24, alphaGain: 0.55 },\n      { mapKey: "36-v2", size: 36 },\n      { mapKey: "48", size: 48, alphaGain: 0.55 },',
  'userscript scan profiles'
);
fs.writeFileSync(userscriptPath, userscript);

let docsApp = fs.readFileSync(docsAppPath, 'utf8');
if (!docsApp.includes('MIN_MULTI_SCORE_GAP')) {
  docsApp = replaceOnce(
    docsApp,
    'const MIN_MULTI_CANDIDATE_SCORE = 0.002;\n',
    'const MIN_MULTI_CANDIDATE_SCORE = 0.002;\nconst MIN_MULTI_SCORE_GAP = 0.0005;\n',
    'docs multi-candidate gap constant'
  );
}
if (!docsApp.includes('const ONE_K_SIZE_KEYS')) {
  docsApp = replaceOnce(
    docsApp,
...
docsApp = replaceOnce(
  ...
  /function getCandidateConfigs\(width, height\) \{[\s\S]*?\n\}\n\nfunction scoreConfig/,
  docsCandidateBlock(),
  'docs candidate resolver'
);
docsApp = patchSelectConfig(docsApp, '', "'");
docsApp = replaceOnce(
  docsApp,
  "    { mapKey: '24-preview', size: 24, alphaGain: 0.55 },\n    { mapKey: '48', size: 48, alphaGain: 0.55 },",
  "    { mapKey: '24-preview', size: 24, alphaGain: 0.55 },\n    { mapKey: '36-v2', size: 36 },\n    { mapKey: '48', size: 48, alphaGain: 0.55 },",
...
tests = replaceOnce(tests, "assert.equal(readVersion(), '0.2.5');", "assert.equal(readVersion(), '0.3.0');", 'test version expectation');
if (!tests.includes("readMap('36-v2').length")) {
  tests = replaceOnce(
    tests,
...
if (!changelog.includes('compact current `36-v2`')) {
...
console.log('Applied Light engine upgrade and CI configuration.');
